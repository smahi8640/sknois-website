...
$cat= $this->db->query("select * from  categories where alias='".$controller."'")->getRowArray(); ?>
...
<main>
    <?php if($cat['banner_image']){ 
        ...
        $style="style='margin-top:180px';";
        $dnone="d-none";
    }
    ?>
        <section class="banner--block <?= $dnone ?>">
        ...
        <section class="breadcum--block category__page" <?= $style ?>>
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
						...
						<form action="" id="search-form" method="GET">
							<div class="filter__main d-flex align-items-center">
                                ...
                                <?php if(!empty($categories)){ ?>
                                    ...
                                    <select class="form-control select2" name="categories[]" data-placeholder="Collection" id="filter_category"  multiple="multiple">
                                        <?php
                                ...
                                <?php if($controller=="all"){ ?>
                                    ...
                                    <select class="form-control select2" name="categories[]" data-placeholder="Collection" id="filter_category"  multiple="multiple">
                                        <?php
                                        $cat = $this->db->query("select * from categories where status='1'")->getResultArray();
                                ...
                                <?php }?>
                                    ...
                                    <div class="custom__select d-none col-lg-2">
                                ...
                                <?php if($controller!="diamond-nose-pin"){ ?>
                                <div class="custom__select col-lg-2">
    ...
    jQuery(document).ready( function () {
        ...
        $.ajax({
            url:'<?php echo base_url()."/categories/ajax/"; ?>'+alias,
            method:"POST",
            data: postForm,
            dataType  : 'json',
            success(response){
        ...
        jQuery("#filter_price, #filter_stock, #filter_category, #filter_size, #filter_lenth, #filter_gender, #filter_short_by").on('change', function(){
            ...
            jQuery("input[name='limit']").val(4);

            var formData = { 
                'filter_price'  : jQuery('#filter_price').val(), 
                'filter_stock'  : jQuery('#filter_stock').val(), 
                'filter_category'  : jQuery('#filter_category').val(), 
                'filter_size'  : jQuery('#filter_size').val(), 
                'filter_lenth'  : jQuery('#filter_lenth').val(), 
                'filter_gender'  : jQuery('#filter_gender').val(), 
                'filter_short_by'  : jQuery('#filter_short_by').val(), 
                'alias'  : '<?= $controller ?>', 
                'start' : jQuery('input[name="start"]').val(),
                'limit' : jQuery('input[name="limit"]').val(),
            };
            ...
            var loadmore = 0;
            loadMoreProducts(formData, loadmore);
        ...
        function loadMoreProducts(formData, loadmore){
            //jQuery(".loader").show();
            //jQuery(".product-list").hide();
            //jQuery(".product-list").attr('loading',true);
